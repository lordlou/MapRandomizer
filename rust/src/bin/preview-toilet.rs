...
fn render_room(room_path: &Path, output_rooms_dir: &Path, tilesets: &HashMap<usize, SCETileset>, draw_tube: bool) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    for theme_dir in std::fs::read_dir(projects_dir)? {
        let theme_dir = theme_dir?;
        let theme_path = theme_dir.path();
        let theme_name = theme_dir.file_name().to_str().unwrap().to_owned();
        println!("Theme: {}", theme_name);
        let theme_rooms_path = theme_path.join("Export/Rooms");
        let output_theme_path = output_rooms_dir.join(&theme_name);
        std::fs::create_dir_all(&output_theme_path)?;
        for room_path in std::fs::read_dir(theme_rooms_path)? {
            let room_path = room_path?.path();
            println!("Room: {}", room_path.to_str().unwrap());
            render_room(&room_path, &output_theme_path, &sce_tilesets, theme_name != "TransitTube")
              .context(format!("Error rendering room: {}", room_path.display()))?;
        }
    }
